...
DATA = "./dataset_triplet/Vimeo-90K/vimeo_triplet/"
RESULT = "./VFI_result/Vimeo-90K/TTVFI/"
GT = "./dataset_triplet/Vimeo-90K/vimeo_triplet/"
...
test_fn = os.path.join(DATA, 'tri_testlist.txt')
with open(test_fn, 'r') as f:
    testlist = f.read().splitlines()[:]
gen_dir = os.path.join(RESULT, 'result')
if not os.path.exists(gen_dir):
    os.mkdir(gen_dir)
for dir in testlist:
    # prepare the image save path
    if os.path.exists(os.path.join(gen_dir, dir)):
        continue
    else:
        os.makedirs(os.path.join(gen_dir, dir))
    First = os.path.join(DATA,'sequences', dir, "im1.png")
    Second = os.path.join(DATA,'sequences', dir, "im3.png")
    Out = os.path.join(gen_dir, dir, "im2.png")
    gt_path = os.path.join(GT,'sequences', dir, "im2.png")
