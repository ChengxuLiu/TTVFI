...
class PWCDCNet(nn.Module):
    ...
    def forward(self,x, output_more = False):
        ...
        flow6 = self.predict_flow6(x)  #4*7
        up_flow6 = self.deconv6(flow6)
        up_feat6 = self.upfeat6(x)
        # print("level6 " +str(time.time()- start))
        # start=  time.time()
        
        warp5 = self.warp(c25, up_flow6*0.625)  #0.625=20/32
        ...
        if not output_more:
            ...
            return [flow2,flow3,flow4,flow5,flow6]#64*112->4*7
        # if self.training:
...
def pwc_dc_net(path=None):
    ...
    if path is not None:
        ...
        if 'state_dict' in data.keys():
            ...
            print("load PWCNet successful state_dict !!!")
        else:
            model.load_state_dict(data)
            print("load PWCNet successful !!!")
    return model
