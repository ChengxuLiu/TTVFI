...
from losses.loss_function import part_loss_s1
...
from models.TTVFI_S1 import CML
...
def train():
    ...
    model = CML(timestep=args.time_step,training=True)
    if args.use_cuda:
    ...
    vimeo_train_set = Vimeo_90K_loader(data_root=args.datasetPath,is_training=True)
    ...
    print("cml lr is: "+ str(float(optimizer.param_groups[1]['lr'])))
    print("EPOCH is: "+ str(int(len(vimeo_train_set) / args.batch_size )))
    ...
    for t in range(args.numEpoch):
        ...
        for i, (img0, gt, img1) in enumerate(vimeo_train_loader):
            ...
            diffs, imgt, img01 = model(torch.stack((img0,gt,img1),dim = 0))
            pixel_loss, census_loss = part_loss_s1(diffs, [img0,img1], imgt, img01 ,epsilon=args.epsilon)
            total_pixel_loss = sum(x*y if x > 0 else 0 for x,y in zip(args.pixel_alpha, pixel_loss))
            total_loss = total_pixel_loss + args.census_alpha * census_loss
        ...
        for i, (img0, gt, img1) in enumerate(vimeo_val_loader):
            ...
            with torch.no_grad():
                ...
                val_total_loss = val_pixel_loss + args.census_alpha * census_loss
        ...
        print("\nEpoch " + str(int(t)) +
              ...
              "\tlearning rate cml: " + str(float(lr_cml)) +
        ...
        auxiliary_data.append(['Epoch:',t, 'PSNR:', val_total_PSNR_loss.avg, 'Vimeo', float(lr_flow),
